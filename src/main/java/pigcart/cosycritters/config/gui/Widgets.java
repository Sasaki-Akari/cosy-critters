...
import static pigcart.cosycritters.CosyCritters.MOD_ID;
...
public class Widgets {
    ...
    public static AbstractWidget getFloatSlider(String name, float initialValue, Consumer<Float> onValueChange, float min, float max, float step, Function<Object, Component> valueFormatter) {
        return new AbstractSliderButton(0, 0, BIG_BUTTON_WIDTH, BUTTON_HEIGHT,
            ...
            protected void applyValue() {
                final float realValue = Math.round(Mth.lerp(value, min, max) / step) * step;
                this.setMessage(Component.translatable(name).append(": ").append(valueFormatter.apply(realValue)));
                onValueChange.accept(realValue);
            }
        };
    }
    public static AbstractWidget getIntSlider(String name, int initialValue, Consumer<Integer> onValueChange, float min, float max, float step, Function<Object, Component> valueFormatter) {
        return new AbstractSliderButton(0, 0, BIG_BUTTON_WIDTH, BUTTON_HEIGHT,
                Component.translatable(name).append(": ").append(valueFormatter.apply(initialValue)),
                Mth.lerp(initialValue, min, max)
        ) {
            ...
            protected void applyValue() {
                final int realValue = (int) (Math.round(Mth.lerp(value, min, max) / step) * step);
                this.setMessage(Component.translatable(name).append(": ").append(valueFormatter.apply(realValue)));
                onValueChange.accept(realValue);
            }
        };
    ...
    private static AbstractWidget[] getRowWidgets(ConfigScreen screen, Field field, String name, Object currentValue, Object defaultValue, Consumer onValueChange, Function<Object, Component> valueFormatter, Class<?> type) {
        if (field.isAnnotationPresent(Slider.class)) {
            ...
            if (type.equals(float.class)) {
                return new AbstractWidget[]{Widgets.getFloatSlider(name, (Float) currentValue, onValueChange, slider.min(), slider.max(), slider.step(), valueFormatter)};
            } else { //if int
                return new AbstractWidget[]{Widgets.getIntSlider(name, (Integer) currentValue, onValueChange, slider.min(), slider.max(), slider.step(), valueFormatter)};
            }
            ...
            return new AbstractWidget[]{
                    Widgets.getButton(Component.translatable(name), (bttn)-> Minecraft.getInstance().setScreen(new ConfirmLinkScreen(
                            (result) -> {
                                if (result) Util.openUri((URI) currentValue);
    ...
    private static Object getNewValue(Class<?> type) {
        if (type.isEnum()) {
            ...
            try {
                ...
                CosyCritters.LOGGER.error("Couldn't get new value for: {}", type.getSimpleName());
                throw new RuntimeException(e);
            }
        }
    }
